    ...
    <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className={`flex items-center justify-between p-4 border-b ${isDark ? "border-white/10" : "border-black/5"}`}>
          ...
          <button
            onClick={handleClose}
            className={`p-1 rounded-full transition-colors ${isDark ? "text-white/50 hover:bg-white/10 hover:text-white" : "text-slate-500 hover:bg-black/5 hover:text-slate-800"
              }`}
          >
            <X className="w-5 h-5" />
          </button>
...
{/* 
  ...
  1. ONLY TEXT ALERT SHOWING (No cancel button)
  <ConfirmationModal
    isOpen={isOpen}
  ...
  2. CONFIRM OR CANCEL (Standard Yes/No)
  <ConfirmationModal
    isOpen={isOpen}
    onClose={() => setIsOpen(false)}
  ...
  3. CONFIRM TEXT WAY (Requires user to type a word to enable confirm button)
  <ConfirmationModal
    isOpen={isOpen}
    onClose={() => setIsOpen(false)}
    onConfirm={() => console.log("Action Executed")}
