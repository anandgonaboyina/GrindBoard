...
import WeekdayBlueprint from '../stats/WeekdayBlueprint';
import SmoothnessGauge from '../stats/SmoothnessGauge';
import PerformanceSidebar from '../stats/PerformanceSidebar';
import FocusHeatmap from '../stats/FocusHeatmap';
import TimelineArchive from '../stats/TimelineArchive';
...
window.addEventListener('wheel', (event) => {
    if (event.deltaY > 0) {
        ...
        // Add your custom animation or action here
    } else {
  ...
  // --- PRE-ASSEMBLE CONTENT BLOCKS TO AVOID DUPLICATION ---
  ...
  const SidebarElements = (
    <>
      <PerformanceSidebar 
        ...
        setShowFriendTimetable={setShowFriendTimetable}
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200" onClick={handleClose}>
      <div className={`relative w-full h-[98vh] md:h-[85vh] max-w-6xl rounded-2xl shadow-2xl overflow-hidden flex flex-col border transition-colors ${isLight ? 'bg-slate-50 border-slate-300' : 'bg-slate-900 border-slate-700/60'}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={`flex-none px-2 py-2.5 flex justify-between items-center border-b shrink-0 z-40 ${isLight ? 'bg-white border-slate-200' : 'bg-slate-900/90 border-slate-800'}`}>
          <div className="w-full flex justify-between items-center ">
            <h2 className={`text-sm md:text-base font-black tracking-tight flex items-center gap-1 ${isLight ? 'text-slate-800' : 'text-slate-100'}`}>
          ...
          <button onClick={handleClose} className={`p-1.5 ml-2 rounded-lg transition-colors ${isLight ? 'bg-slate-100 hover:bg-slate-200 text-slate-500' : 'bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-slate-200'}`}>
