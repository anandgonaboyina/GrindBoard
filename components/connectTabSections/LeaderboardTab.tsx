...
export default React.memo(function LeaderboardTab({ setSelectedImageOverlay }: LeaderboardTabProps) {
  ...
  // 1. We use useCallback so this helper function doesn't get recreated every render
  ...
  // useMemo does the heavy sorting & filtering ONLY when data/search/filters change
  const sortedAndFilteredData = useMemo(() => {
    ...
    const sorted = [...leaderboardData].sort((a, b) => {
      ...
      // Primary: Sort by selected period focus time (descending)
      if (valA !== valB) return valB - valA;

      // Tie breaker: wakeupTime (earlier is better, missing is worst)
    ...
    // B. Filter the data by search
    if (!leaderboardSearch.trim()) return sorted;
    
    return sorted.filter(u => 
      u.displayName.toLowerCase().includes(leaderboardSearch.toLowerCase())
    );
